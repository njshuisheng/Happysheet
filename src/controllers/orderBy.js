...
export function orderByInitial() {
  ...
  //菜单栏 排序按钮
  $("#luckysheetorderbyasc, #luckysheetorderbyasc_t").mousedown(function (
    event
  ) {
  ...
  $("#luckysheetorderby").click(function () {
    ...
    if (luckysheet_sort_initial) {
      ...
      $("#luckysheet-sort-dialog .luckysheet-sort-dialog-additem").click(
        function () {
          let last = Store.luckysheet_select_save[0];
          let r1 = last["row"][0],
            r2 = last["row"][1];
          ...
          let option = "",
            i = $("#luckysheet-sort-dialog table").data("itemcount") + 1;
          ...
          for (let c = c1; c <= c2; c++) {
            if (t) {
              ...
              option +=
                '<option value="' + c + '">' + chatatABC(c) + "</option>";
            }
          }
      ...
      $("#luckysheet-sort-modal-confirm").click(function () {
        ...
        $($("#luckysheet-sort-dialog table tr").toArray().reverse()).each(
          function () {
    ...
    $("#luckysheet-sort-dialog .luckysheet-modal-dialog-title-text").html(
      locale_sort.sortRangeTitle +
        "<span>" +
        chatatABC(c1) +
        (r1 + 1) +
        "</span>" +
        locale_sort.sortRangeTitleTo +
        "<span>" +
        chatatABC(c2) +
        (r2 + 1) +
        "</span>"
