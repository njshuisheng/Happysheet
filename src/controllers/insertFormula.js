...
const insertFormula = {
  init: function () {
    ...
    $(document)
      ...
      .on(
        "click.fxFormulaCf",
        "#luckysheet-search-formula-confirm",
        function () {
          let formula = $("#luckysheet-search-formula .listBox.on").attr(
            "name"
          );
          let formulaTxt =
    ...
    $(document)
      ...
      .on(
        "focus.fxParamInput",
        "#luckysheet-search-formula-parm .parmBox input",
        function () {
          let parmIndex = $(this).parents(".parmBox").index();
          formula.data_parm_index = parmIndex;

          let formulatxt = $(this)
            .parents("#luckysheet-search-formula-parm")
            .find(".luckysheet-modal-dialog-title-text")
            .text();
          ...
          if (parmIndex >= parmLen) {
            parmDetail =
              Store.luckysheet_function[formulatxt].p[parmLen - 1].detail;
            parmRepeat =
              Store.luckysheet_function[formulatxt].p[parmLen - 1].repeat;
          } else {
            parmDetail =
              Store.luckysheet_function[formulatxt].p[parmIndex].detail;
            parmRepeat =
              Store.luckysheet_function[formulatxt].p[parmIndex].repeat;
          }
          ...
          ).appendTo($("#luckysheet-search-formula-parm .parmDetailsBox"));
    ...
    $(document)
      ...
      .on(
        "click.fxParamI",
        "#luckysheet-search-formula-parm .parmBox i",
        function () {
          ...
          if ($(this).parents(".parmBox").find(".txt input").val() == "") {
            $("body.object-design").append(
              replaceHtml(modelHTML, {
                ...
                content:
                  "<input id='luckysheet-search-formula-parm-select-input' class='luckysheet-datavisual-range-container' style='font-size: 14px;padding:5px;max-width:none;' spellcheck='false' aria-label='" +
                  locale_formulaMore.tipDataRangeTile +
                  "' readonly='true' placeholder='" +
                  locale_formulaMore.tipDataRangeTile +
                  "'>",
                botton:
                  '<button id="luckysheet-search-formula-parm-select-confirm" class="btn btn-primary">' +
    ...
    $(document)
      ...
      .on(
        "click.fxParamCf",
        "#luckysheet-search-formula-parm-confirm",
        function () {
    ...
    $(document)
      ...
      .on(
        ...
        "#luckysheet-search-formula-parm-select-confirm",
        function () {
          let parmIndex = $(
            "#luckysheet-search-formula-parm-select-input"
          ).attr("data_parm_index");
  ...
  formulaListDialog: function () {
    ...
    $("body.object-design").append(
      replaceHtml(modelHTML, {
        ...
        botton:
          '<button id="luckysheet-search-formula-confirm" class="btn btn-primary">' +
          locale_button.confirm +
  ...
  formulaParmDialog: function (formulaTxt, parm) {
    ...
    $("body.object-design").append(
      replaceHtml(modelHTML, {
        ...
        botton:
          '<button id="luckysheet-search-formula-parm-confirm" class="btn btn-primary">' +
          locale_button.confirm +
          '</button><button class="btn btn-default luckysheet-model-close-btn">' +
          locale_button.cancel +
          "</button>",
  ...
  functionStrCompute: function () {
    ...
    let formulatxt = $("#luckysheet-search-formula-parm")
      .find(".luckysheet-modal-dialog-title-text")
      .text();
    ...
    if (isVal) {
      //公式计算
      let fp = $.trim(
        formula.functionParserExe($("#luckysheet-rich-text-editor").text())
      );
