...
const imageCtrl = {
  ...
  _insertImg: function (src) {
    ...
    image.src =
      typeof imageUrlHandle === "function" ? imageUrlHandle(src) : src;
  },
  ...
  modelHtml: function (id, imgItem) {
    ...
    return `<div id="${id}" class="luckysheet-modal-dialog luckysheet-modal-dialog-image" style="width:${width}px;height:${height}px;padding:0;position:${position};left:${left}px;top:${top}px;z-index:200;">
                    <div class="luckysheet-modal-dialog-content" style="width:100%;height:100%;overflow:hidden;position:relative;">
                        <img src="${src}" style="position:absolute;width:${
    ...
    } ${imgItem.border.color};border-radius:${
  ...
  sliderHtmlShow: function () {
    ...
    $("body#object-design").append(sliderHtml);
    luckysheetsizeauto();
  ...
  init: function () {
    ...
    $("#luckysheet-image-showBoxs")
      ...
      .on(
        "mousedown.restore",
        ".luckysheet-modal-controll-restore",
        function (e) {
  ...
  resizeImgItem: function () {
    ...
    item.default.height = Math.round(
      (item.default.height * scaleY) / zoomRatio
    );

    item.crop.width = Math.round((item.crop.width * scaleX) / zoomRatio);
    item.crop.height = Math.round((item.crop.height * scaleY) / zoomRatio);
    item.crop.offsetLeft = Math.round(
      (item.crop.offsetLeft * scaleX) / zoomRatio
    );
    item.crop.offsetTop = Math.round(
      (item.crop.offsetTop * scaleY) / zoomRatio
    );
