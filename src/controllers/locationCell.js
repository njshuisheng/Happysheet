...
const luckysheetLocationCell = {
  createDialog: function () {
    ...
    let content =
      ...
      '<label for="locationNull">' +
      locale_location.locationNull +
      "</label>" +
      "</div>" +
      '<div class="listItem">' +
      '<input type="radio" name="locationType" id="locationCF">' +
      '<label for="locationCF">' +
      locale_location.locationCondition +
      "</label>" +
      "</div>" +
      '<div class="listItem">' +
      '<input type="radio" name="locationType" id="locationStepRow">' +
      '<label for="locationStepRow">' +
      locale_location.locationRowSpan +
      "</label>" +
      "</div>" +
      '<div class="listItem">' +
      '<input type="radio" name="locationType" id="locationStepColumn">' +
      '<label for="locationStepColumn">' +
      locale_location.locationColumnSpan +
      "</label>" +
      "</div>" +
      "</div>";
    ...
    $("body.object-design").append(
      replaceHtml(modelHTML, {
        ...
        content: content,
        botton:
          '<button id="luckysheet-locationCell-dialog-confirm" class="btn btn-primary">' +
  ...
  init: function () {
    ...
    $(document).on(
      ...
      function (e) {
        ...
        $("#luckysheet-locationCell-dialog .listItem .subbox label").css(
          "color",
          "#666"
        );
    ...
    $(document)
      .off("click.locationCellConfirm")
      .on(
        "click.locationCellConfirm",
        "#luckysheet-locationCell-dialog #luckysheet-locationCell-dialog-confirm",
        function () {
          $("#luckysheet-modal-dialog-mask").hide();
          $("#luckysheet-locationCell-dialog").hide();

          let $radio = $(
            "#luckysheet-locationCell-dialog .listItem input:radio:checked"
          );
        ...
        for (let r = st_r; r <= ed_r; r++) {
          for (let c = st_c; c <= ed_c; c++) {
            ...
            if (
              ...
              cell.f != null &&
              (value == "all" ||
                (cell.ct != null && value.indexOf(cell.ct.t) > -1))
            ) {
              cellSave[r + "_" + c] = 0;
            } else if (
              type == "locationConstant" &&
